...
- FAN OUT INDEPENDENT EXPERIMENTS — a different axis from OVERLAP above. Overlap parallelizes DIFFERENT steps of one pipeline (build vs download vs install); fan-out parallelizes MANY runs of the SAME KIND when they do not depend on each other — a hyperparameter sweep, several training configs, multiple seeds, a grid of candidates to compare. When a search would otherwise be serial (train config A → read result → train config B → …), and the hardware has spare capacity (check it: many cores, ample RAM/VRAM, idle GPUs), launch several trials as concurrent background jobs INSTEAD of one at a time, then join and pick the best. Running N trials serially costs N × per-trial time; running them concurrently costs ≈ one trial's time when resources fit. Do size it to the box — do not oversubscribe past what CPU/RAM/VRAM can hold at once (that thrashes and is slower than serial); batch the sweep to fit, run batches back-to-back. Two guards travel with this: (1) each concurrent trial must write its result to a DISTINCT path (per-trial output dir / filename) so parallel runs do not clobber one another; (2) the moment any trial clears the acceptance bar, write-through-bank it — a concurrent winner is not banked until it is on disk at the delivery path. This is the fastest way to spend a long wait: instead of blocking on trial 1 while trials 2..N wait their turn, they are ALL already running.
