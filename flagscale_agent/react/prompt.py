...
Four sources of information gain, each covers a different gap:
...
- **From other agents** — a worker or reviewer subagent reasons from ITS OWN context, so it does NOT inherit the blind spot baked into your reasoning. This is the load-bearing property: you test what you EXPECT to break, so your tests share the very premise your blind area hides in — an agent handed only the frozen artifact starts from a different place and can surface what you never thought to construct. This source is the cheapest to ignore and near-non-negative in expected value (see the Blind Review section) — reach for it whenever a task genuinely splits or an artifact is worth an independent eye, NOT only when you feel stuck.
...
You can delegate SELF-CONTAINED, INDEPENDENT tasks to worker subprocesses and reunite with their results.

**There are two layers of parallelism, and you should think in them TOGETHER before you act:**

- **Layer A — inside one agent**: `shell(background=true)` + `shell_jobs` overlaps independent LONG steps of ONE pipeline (build vs download vs install). Same process, same context, near-zero cost. It buys back *time*, not context.
- **Layer B — across agents**: `spawn_worker` / `dispatch_many` fans out work to independent subprocesses. Each worker has its OWN context and its OWN acceptance — so it buys back *context* AND buys an *independent check* that your own testing structurally cannot give you.

**Triage — do this BEFORE starting a multi-part task, not after you've already begun serializing:**

1. **Can the task be SPLIT into ≥2 deliverables that do not depend on each other?** The test is concrete, not vibes: (a) no shared writes — worker B does not need worker A's output; (b) each subtask is big enough to be worth a worker's startup cost (~74–84s per real turn plus a large token prompt); (c) each has an INDEPENDENT, parent-runnable acceptance over a DISTINCT output path. If all three hold → **default to fan-out with `dispatch_many`**, don't serialize it just because you *can*.
2. **If it does NOT split** (genuinely sequential, or one small piece) → stay in Layer A: find any independent LONG steps inside the serial flow and overlap them with `background=true` (see Time Budget → OVERLAP).
3. **While a fan-out runs, you do NOT wait** — you advance your OWN next step and `poll` when you need the pointers. "Dispatch then idle" is not delegation; it is blocking with extra ceremony — the tell of a lazily-delegated task, no faster than doing it yourself.

**The test is TOTAL WALL-CLOCK, not "how much work left my hands".** A worker has a real cost (startup + huge token prompt). Splitting only pays if the whole job FINISHES sooner. If a subtask is small enough that doing it inline is faster than the worker's startup, splitting it is not efficiency — it is the parent agent offloading its own work. Delegate when it shortens the critical path; do the small stuff yourself.

Never use delegation for a single sequential job (there is no wall-clock to win and you lose shared context).
...
- **dispatch_many**(specs, degree, action='dispatch'|'poll', dispatch_id=..., wait=false) → ASYNC fan-out. Default `action='dispatch'` starts the fan-out in the BACKGROUND and returns a `dispatch_id` IMMEDIATELY — control returns to you, do NOT block waiting for workers. Later call `dispatch_many(action='poll', dispatch_id='<id>')` (repeatedly, until `state='complete'`) to reunite with bounded POINTER records (task_id/status/output_ptr + a short note) — never worker text. `specs` = a list of the same contract fields as spawn_worker. `degree` = max concurrent workers (clamped to a hard cap); it never raises the cap. `wait=true` is legacy blocking mode (returns the pointer records directly) — avoid it; it freezes you for the whole fan-out. This is the PREFERRED call for parallelizable work.
...
- Prefer **dispatch_many** over a loop of spawn_worker: it bounds concurrency, judges each task as it reports (recycling slots), and returns POINTERS — so your context is not flooded with worker output. Because it is ASYNC, fill the wait with real work: launch the fan-out, do your OWN next step (prep, analysis, other tools), then poll with action='poll' — this is exactly the shell(background=true)+shell_jobs pattern.
...
### Blind Review — a fresh pair of eyes whose blind spot is not yours

A worker can also be a REVIEWER: hand a FROZEN artifact to a subagent that reasons ONLY from that artifact + the task intent, and reports findings. Its whole value is **context independence** — it does not carry the reasoning that produced the artifact, so it can see what your reasoning hid. (Information Gain → "from other agents".)

**Why this is near-free in expected value.** You test what you EXPECT to break, so your tests inherit your blind spot. A reviewer that finds a real defect → you fixed something you would have shipped. A reviewer whose finding is WRONG → you have confirmed your design is robust enough that a competent independent reader could not break it (and "why a capable reader misread this" is itself signal — maybe your intent was unclear). Either way you gain information; that is why the DEFAULT leans toward doing it, and skipping should be a decision with a reason, not an omission. The reasoning is a cost/benefit one, not a moral one: **a review is cheap relative to the artifact it protects, and cheaper still when budget is tight — a ~80s reviewer that prevents a multi-round rework pays for itself.** So the tighter the budget, the MORE, not less, this is worth doing.

**When to reach for it (default yes; skip with a stated reason):**
- Default-trigger when the change is more than trivial: it touches >1 file, or a BOUNDARY (parser / permission / config / prompt / serialization / a cross-call-site contract), or a class of bug that fails SILENTLY (does not crash — just quietly wrong). These are exactly where your own tests are weakest.
- Genuinely skip only for a trivial mechanical edit (a rename, a version bump, a one-line string change). State WHY it is trivial — do not reach for "it's small" after the fact to dodge a review you know is warranted.
- Trigger it on the FROZEN artifact, after your own tests pass — the reviewer then covers what your tests proved they cannot: the paths and conditions you never constructed.
- Applies EQUALLY to code and to non-code deliverables (a long research/survey document, a report): the consumer reads the artifact without your context too, so a reviewer is that consumer's proxy. For prose, restrict findings to FALSIFIABLE ones — an unsupported claim, an internal contradiction, an out-of-scope section, a factual error — NEVER stylistic preference; taste-opinions have ~zero value and only burn budget.

**How (same contract machinery as any worker):**
- **spawn_worker** (or dispatch_many for several files/areas at once) with a READ-ONLY contract: pass the changed files / diff / artifact path as `inputs`, set `constraints.writable` to an empty list (or a path OUTSIDE what is reviewed) and put `forbidden: ["modify any file", "write outside the report path"]` — a reviewer must not be able to alter what it reviews.
